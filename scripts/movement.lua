...
function movement.queue(machine, goal, purpose)
  ...
  machine.controller.path_deferrals = 0
...
function movement.on_path_finished(event)
  ...
  if event.path and #event.path > 0 then
    ...
    if request.purpose == "entrance" then
      -- The validated route is clear in this deliberately small slice. Steering
      -- toward its exact end avoids oscillating across one-tile path nodes.
      machine.controller.waypoints = {{position = request.goal}}
    else
...
local function follow(machine, entity, tick)
  ...
  while distance(entity.position, target) <= ARRIVAL_RADIUS do
  ...
  local lookahead_index = math.min(controller.waypoint_index + (controller.purpose == "entrance" and 5 or 0), #controller.waypoints)
  local lookahead = controller.waypoints[lookahead_index]
  local steering_target = lookahead.position or lookahead
  local turn = orientation_delta(entity.orientation, desired_orientation(entity.position, steering_target))
...
function movement.update(machine, tick)
  ...
  if controller.state == "following" or controller.state == "working" then
  ...
  elseif controller.state == "deferred" and tick >= controller.retry_tick then
    movement.queue(machine, controller.goal, controller.purpose)
  end
  return nil
end
